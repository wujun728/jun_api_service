    ...
    public Result list(@PathVariable("entityName") String entityName,@PathVariable("action") String action, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            if(ObjectUtil.isNotEmpty(apiSql)){
                if("one".equalsIgnoreCase(action) || "page".equalsIgnoreCase(action)
                        || "list".equalsIgnoreCase(action)  || "query".equalsIgnoreCase(action) ){
                    List<Map<String, Object>> datas = SqlXmlUtil.query(DataSourcePool.getConnection(main),apiSql.getText(),parameters);
                    return Result.success(datas);
                }else if("insert".equalsIgnoreCase(action) || "update".equalsIgnoreCase(action)  || "delete".equalsIgnoreCase(action)){
                    int flag = SqlXmlUtil.update(DataSourcePool.getConnection(main),apiSql.getText(),parameters);
                    if(flag>0){
                    ...
                    Long datas = SqlXmlUtil.count(DataSourcePool.getConnection(main),apiSql.getText(),parameters);
                    return Result.success(datas);
                }
                else {
                    Object obj = SqlXmlUtil.executeSql(DataSourcePool.getConnection(main),apiSql.getText(),parameters);
