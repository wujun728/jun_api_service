...
public class RestApiController {
    ...
    //@ApiOperation(value = "根据id删除实体数据" )
    public Result delete1(@PathVariable("entityName") String entityName,@PathVariable("ids") String ids, HttpServletRequest request) throws Exception {
        return Result.success("删除成功！");
    }
    @RequestMapping(path = "/{entityName}/delete", produces = "application/json")
    //@ApiOperation(value = "根据id删除实体数据" )
    public Result delete2(@PathVariable("entityName") String entityName, HttpServletRequest request) throws Exception {
        Map<String, Object> parameters = HttpRequestUtil.getAllParameters(request);
        main = MapUtil.getStr(parameters, "ds","main");
        String tableName = StrUtil.toUnderlineCase(entityName);
        try {
            ...
            Object[] argsDeleteIds = new Object[]{} ;
            Object ids = parameters.get("ids");
            Object id = parameters.get("id");
            if(ObjectUtil.isNotEmpty(ids)){
                if(String.valueOf(ids).contains(",")){
                    argsDeleteIds = String.valueOf(ids).split(",");
                }else{
                    argsDeleteIds = new Object[]{ids};
                }
            }else if(ObjectUtil.isNotEmpty(id)){
                argsDeleteIds = new Object[]{id};
            }else {
                primaryKey = RestUtil.getTablePrimaryKes(table);
                List args = RestUtil.getPrimaryKeyArgs(parameters, table);
                argsDeleteIds = args.toArray();
            }
            Boolean flag =   Db.deleteById(tableName, primaryKey, argsDeleteIds);
            if (flag) {
